...
def get_summary() -> Dict[str, Any]:
    ...
    cur.execute("SELECT COUNT(*) FROM frames"); total_frames = cur.fetchone()[0]
    try:
        cur.execute("SELECT COUNT(*) FROM ocr_index"); indexed_frames = cur.fetchone()[0]
...
def list_videos() -> List[Dict[str, Any]]:
    ...
    """
    con = _conn(); cur = con.cursor()
    cur.execute(q); rows = cur.fetchall(); con.close()
    out = []
    for r in rows:
...
def list_video_frames(video_id: str, limit: int, offset: int, expires_in: int) -> Dict[str, Any]:
    ...
    """
    con = _conn(); cur = con.cursor()
    cur.execute(q, (video_id, limit, offset)); rows = cur.fetchall(); con.close()
    items = []
    for r in rows:
        key = r["frame_path"]
        items.append({
